...
fn test_create_agreement_with_token() {
    ...
    client.add_supported_token(&token_addr, &String::from_str(&env, "USDC"), &6, &1, &1000000000);
...
fn test_make_payment_with_different_token() {
    ...
    let tenant = Address::generate(&env);
    let landlord = Address::generate(&env);
    let base_token = env.register_stellar_asset_contract_v2(Address::generate(&env)).address();
    let pay_token = env.register_stellar_asset_contract_v2(Address::generate(&env)).address();
    
    client.add_supported_token(&base_token, &String::from_str(&env, "USDC"), &6, &1, &1000000000);
    client.add_supported_token(&pay_token, &String::from_str(&env, "EURT"), &6, &1, &1000000000);
