...
pub fn add_supported_token(
    ...
    let mut tokens: Vec<Address> = env.storage().persistent().get(&DataKey::SupportedTokens).unwrap_or(Vec::new(&env));
...
pub fn get_supported_tokens(
    env: Env,
) -> Result<Vec<SupportedToken>, RentalError> {
    let addresses: Vec<Address> = env.storage().persistent().get(&DataKey::SupportedTokens).unwrap_or(Vec::new(&env));
    let mut tokens = Vec::new(&env);
    
    for addr in addresses.iter() {
        if let Some(token) = env.storage().persistent().get::<DataKey, SupportedToken>(&DataKey::SupportedToken(addr)) {
            if token.enabled {
...
pub fn is_token_supported(
    env: Env,
    token_address: Address,
) -> Result<bool, RentalError> {
    let key = DataKey::SupportedToken(token_address);
    if let Some(token) = env.storage().persistent().get::<DataKey, SupportedToken>(&key) {
        Ok(token.enabled)
    } else {
        Ok(false)
    }
...
pub fn get_exchange_rate(
    ...
    if let Some(rate_struct) = env.storage().persistent().get::<DataKey, TokenExchangeRate>(&key) {
        Ok(rate_struct.rate)
    } else {
...
pub fn convert_amount(
    ...
    // amount * rate / 10^18
    let converted = amount.checked_mul(rate).ok_or(RentalError::ConversionError)? / 1_000_000_000_000_000_000;
    Ok(converted)
}
