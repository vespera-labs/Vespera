...
import { UUID, ISO8601DateTime, StellarPublicKey, TransactionHash } from './common.types';

export type AgreementStatus = 'draft' | 'pending_deposit' | 'active' | 'expired' | 'terminated' | 'disputed';
export type TerminationReason = 'lease_end' | 'early_termination_tenant' | 'early_termination_landlord' | 'eviction' | 'mutual_agreement';
...
export interface Dispute {
  ...
  // Dispute details
  type: 'security_deposit' | 'property_damage' | 'lease_violation' | 'payment' | 'other';
  subject: string;
  description: string;
