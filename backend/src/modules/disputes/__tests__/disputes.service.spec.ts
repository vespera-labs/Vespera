...
describe('DisputesService', () => {
  ...
  beforeEach(async () => {
    ...
    agreementRepository = module.get<Repository<RentAgreement>>(getRepositoryToken(RentAgreement));
  ...
  describe('createDispute', () => {
    ...
    it('should create a dispute successfully', async () => {
      ...
      jest.spyOn(queryRunner.manager, 'create')
      ...
      expect(queryRunner.manager.create).toHaveBeenCalledWith(Dispute, expect.objectContaining({
        disputeType: DisputeType.RENT_PAYMENT,
        requestedAmount: 500,
        description: 'Test dispute description',
        status: DisputeStatus.OPEN,
    ...
    it('should throw NotFoundException if agreement not found', async () => {
      ...
      await expect(service.createDispute(createDisputeDto, 'user-1'))
    ...
    it('should throw ForbiddenException if user not party to agreement', async () => {
      ...
      await expect(service.createDispute(createDisputeDto, 'other-user'))
  ...
  describe('findOne', () => {
    it('should return a dispute when found', async () => {
      jest.spyOn(disputeRepository, 'findOne')
        .mockResolvedValue(mockDispute);
  ...
  describe('isValidStatusTransition', () => {
    it('should validate correct status transitions', () => {
      jest.spyOn(service, 'isValidStatusTransition' as any)
        .mockReturnValue(true);
      expect(service['isValidStatusTransition'](DisputeStatus.OPEN, DisputeStatus.UNDER_REVIEW))
        .toBe(true);
      expect(service['isValidStatusTransition'](DisputeStatus.UNDER_REVIEW, DisputeStatus.RESOLVED))
        .toBe(true);
    });
    ...
    it('should reject invalid status transitions', () => {
      ...
      expect(service['isValidStatusTransition'](DisputeStatus.RESOLVED, DisputeStatus.OPEN))
        .toBe(false);
      expect(service['isValidStatusTransition'](DisputeStatus.OPEN, DisputeStatus.RESOLVED))
        .toBe(false);
    });
  });
});
